...
import { marketData as staticMarketData, categories, type MarketTrend } from "@/data/marketData";
...
export default function MarketAnalysis() {
  ...
  const handleRefreshTrends = useCallback(async () => {
    ...
    try {
      ...
      toast.success("🔄 تم تحديث التراندات بنجاح من AI!");
    } catch (error) {
      const errType = classifyGeminiError(error);
      if (errType === "no_key") {
        toast.error("أضف مفتاح API من الإعدادات ⚙️");
      } else {
        toast.error("تعذر تحديث التراندات. تم الاحتفاظ بالبيانات الحالية.");
      }
    } finally {
      setRefreshing(false);
    }
  ...
  const handleAnalyze = async (topic: string) => {
    ...
    setAiAnalysis("");
    try {
      const result = await getAIMarketAnalysis(topic);
      setAiAnalysis(result);
    } catch {
      toast.error("خطأ في التحليل. تأكد من صحة مفتاح API في الإعدادات.");
    } finally {
      setAnalyzing(false);
    }
  };
    ...
    <div className="animate-fade-in space-y-5">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        >
          {refreshing ? (
            ...
            "🔄 تحديث التراندات بالذكاء الاصطناعي"
          )}
        </button>
      ...
      <div className="bg-card border-2 border-primary rounded-lg p-4 box-glow">
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className={`px-3 py-1.5 rounded text-xs font-mono font-semibold border-2 transition-all ${
              autoPulse
                ...
                : "bg-card text-secondary border-primary/40"
            }`}
          >
            {autoPulse ? "🟢 Auto Pulse ON" : "⚪ Auto Pulse OFF"}
          </button>
          <label className="text-primary text-xs font-mono">كل</label>
          <select
            value={pulseSeconds}
            onChange={(e) => setPulseSeconds(Number(e.target.value))}
            className="bg-card border border-primary/30 text-primary px-2 py-1 rounded text-xs font-mono"
          >
          ...
          <span className="text-secondary text-[10px] font-mono">تحديث تلقائي مستمر للتراندات ومؤشرات المصادر</span>
        </div>
      </div>
      ...
      <div className="bg-card border-2 border-primary rounded-lg p-5 box-glow overflow-x-auto">
        <h3 className="text-base font-semibold text-primary text-glow mb-4 font-mono">
          📊 التراندات والفرص ({filtered.length} نتيجة)
        </h3>
                ...
                <tr key={`${item.topic}-${idx}`} className="hover:bg-primary/5 transition-colors border-b border-primary/20">
                  ...
                  <td className="p-2.5">
                    <span className={`inline-block px-2.5 py-1 rounded-sm text-[11px] font-semibold border font-mono ${
                      item.profitability >= 85 ? "bg-primary/20 text-primary border-primary" :
                      item.profitability >= 75 ? "bg-cyber-yellow/20 text-cyber-yellow border-cyber-yellow" :
                  ...
                  <td className="p-2.5">
                    <button
                      onClick={() => handleAnalyze(item.topic)}
                      className="px-3 py-1 rounded text-[11px] font-mono font-semibold gradient-primary text-primary-foreground hover:box-glow-strong transition-all"
                    >
                      🧠 تحليل
                    </button>

                    <button
                      onClick={() => setOneClickTrend(item)}
                      className="ml-2 px-3 py-1 rounded text-[11px] font-mono font-semibold bg-accent/20 text-accent border border-accent/30 hover:bg-accent/25 hover:box-glow transition-all"
                      title="تحليل + توليد برومبت بضغطة واحدة"
                    >
                      ⚡ One-Click
                    </button>
                  </td>
        ...
        {filtered.length === 0 && (
          <div className="text-center text-secondary font-mono text-sm py-8">لا توجد نتائج مطابقة للفلاتر المحددة</div>
        )}
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {sourcePulse.map((s) => (
            <a
              ...
              className="no-underline bg-primary/5 border border-primary/20 rounded-md p-3 hover:bg-primary/10 transition-all"
            >
              <div className="flex items-center justify-between">
                <span className="text-primary font-mono text-xs font-semibold">{s.name}</span>
                <span className={`text-[10px] font-mono ${
                  s.status === "live" ? "text-primary" : s.status === "delayed" ? "text-cyber-yellow" : "text-secondary"
                }`}>
                  {s.status === "live" ? "LIVE" : s.status === "delayed" ? "DELAYED" : "MANUAL"}
                </span>
              </div>
              <div className="mt-2 text-secondary font-mono text-[10px]">
                Topic: <span className="text-primary">{s.detectedTopic}</span>
              </div>
              <div className="mt-1 text-secondary font-mono text-[10px]">
