...
// ============================================================================
// 🎨 DESIGN TOKENS - عناصر التصميم القابلة للتعديل
// ============================================================================
...
export const videoConfig = {
  ...
  height: 2160,
  fps: 30,
  durationInSeconds: designTokens.animation.duration,
};
...
export const SVGFilters: React.FC = () => (
  <defs>
    <filter id="glow" x="-50%" y="-50%" width="200%" height="200%">
      <feGaussianBlur stdDeviation={designTokens.effects.glow.spread} result="coloredBlur" />
      <feMerge>
    ...
    <filter id="shadow" x="-50%" y="-50%" width="200%" height="200%">
      <feDropShadow dx={designTokens.effects.shadow.offsetX} dy={designTokens.effects.shadow.offsetY} stdDeviation={designTokens.effects.shadow.blur} floodOpacity={designTokens.effects.shadow.opacity} />
    </filter>
    <filter id="blur">
      <feGaussianBlur stdDeviation={designTokens.effects.blur.amount} />
    </filter>
    <linearGradient id="primaryGradient" x1="0%" y1="0%" x2="100%" y2="100%">
      <stop offset="0%" stopColor={designTokens.colors.primary} />
      <stop offset="100%" stopColor={designTokens.colors.secondary} />
    </linearGradient>
    <radialGradient id="radialGlow" cx="50%" cy="50%" r="50%">
      <stop offset="0%" stopColor={designTokens.colors.accent} stopOpacity="0.8" />
...
interface CircleProps {
  index: number;
  config: typeof designTokens.shapes.circles[0];
  frame: number;
  durationInFrames: number;
}
const AnimatedCircle: React.FC<CircleProps> = ({ index, config, frame, durationInFrames }) => {
  ...
  return (
    <circle cx={x} cy={y} r={config.size / 2} fill={designTokens.colors.gradients[0].start} opacity={opacity} filter="url(#glow)" style={{ transform: `scale(${scale})`, transformOrigin: `${x}px ${y}px` }} />
  );
};

interface RectangleProps {
  index: number;
  config: typeof designTokens.shapes.rectangles[0];
  frame: number;
  durationInFrames: number;
}
const AnimatedRectangle: React.FC<RectangleProps> = ({ index, config, frame, durationInFrames }) => {
  const centerX = 1920;
  const centerY = 1080;
...
interface TriangleProps {
  index: number;
  config: typeof designTokens.shapes.triangles[0];
  frame: number;
  durationInFrames: number;
}
const AnimatedTriangle: React.FC<TriangleProps> = ({ index, config, frame, durationInFrames }) => {
  ...
  return (
    <path d={path} fill={designTokens.colors.gradients[1].start} opacity={opacity} filter="url(#glow)" style={{ transform: `rotate(${rotation}deg) scale(${scale})`, transformOrigin: `${centerX}px ${centerY}px` }} />
  );
};

interface LineProps {
  index: number;
  config: typeof designTokens.shapes.lines[0];
  frame: number;
  durationInFrames: number;
}
const AnimatedLine: React.FC<LineProps> = ({ index, config, frame, durationInFrames }) => {
  const startX1 = 0;
  ...
  return (
    <line x1={x1} y1={y1} x2={x2} y2={y2} stroke={designTokens.colors.primary} strokeWidth={config.strokeWidth} opacity={opacity} strokeLinecap="round" />
  );
};
...
// ============================================================================
const AnimatedBackground: React.FC<{ frame: number; durationInFrames: number }> = ({ frame, durationInFrames }) => {
  return (
    <rect x="0" y="0" width="3840" height="2160" fill={designTokens.colors.background}>
      <animate attributeName="fill" values={`${designTokens.colors.background};#1a1a3e;${designTokens.colors.background}`} dur={`${designTokens.animation.duration}s`} repeatCount="indefinite" />
    </rect>
  );
};
...
export const MyComposition: React.FC = () => {
  ...
  const springConfig = {
    fps,
    config: {
      damping: designTokens.animation.easing.damping,
      stiffness: designTokens.animation.easing.stiffness,
      mass: designTokens.animation.easing.mass,
    },
  };
  
  const titleScale = spring({ frame: frame - designTokens.animation.delays.primary * fps, ...springConfig });
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: designTokens.colors.background }}>
      <svg viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', height: '100%', position: 'absolute' }}>
        <SVGFilters />
        <AnimatedBackground frame={frame} durationInFrames={durationInFrames} />
        
        <g id="circles-layer">
          {designTokens.shapes.circles.map((config, index) => (
            <AnimatedCircle key={`circle-${index}`} index={index} config={config} frame={frame} durationInFrames={durationInFrames} />
          ))}
        </g>
        
        <g id="rectangles-layer">
          {designTokens.shapes.rectangles.map((config, index) => (
            <AnimatedRectangle key={`rect-${index}`} index={index} config={config} frame={frame} durationInFrames={durationInFrames} />
          ))}
        </g>
        
        <g id="triangles-layer">
          {designTokens.shapes.triangles.map((config, index) => (
            <AnimatedTriangle key={`triangle-${index}`} index={index} config={config} frame={frame} durationInFrames={durationInFrames} />
          ))}
        </g>
        
        <g id="lines-layer">
          {designTokens.shapes.lines.map((config, index) => (
            <AnimatedLine key={`line-${index}`} index={index} config={config} frame={frame} durationInFrames={durationInFrames} />
          ))}
        </g>
        ...
        <g id="effects-layer">
          {Array.from({ length: 20 }).map((_, i) => {
            ...
            return <circle key={`dot-${i}`} cx={x} cy={y} r={radius} fill={designTokens.colors.accent} opacity={0.5} filter="url(#glow)" />;
          })}
        </g>
      </svg>
      
      <div style={{ position: 'absolute', top: '50%', left: '50%', transform: `translate(-50%, -50%) scale(${titleScale})`, opacity: contentOpacity, textAlign: 'center' }}>
        <h1 style={{ fontSize: '120px', fontFamily: 'system-ui, sans-serif', fontWeight: 700, color: designTokens.colors.foreground, textShadow: `0 0 40px ${designTokens.colors.primary}`, letterSpacing: '-2px' }}>
          Abstract Background
        </h1>
        <p style={{ fontSize: '32px', color: designTokens.colors.primary, marginTop: '20px', opacity: 0.8 }}>
          4K UHD • 30fps • Remotion
        </p>
      </div>
    </AbsoluteFill>
  );
